    ...
    try:
        ...
        if request.method == "POST":
            ...
            try:
                # Grup oluştur
                c.execute(
                    f"INSERT INTO rooms (room_name, creator, description) VALUES ({PLACEHOLDER}, {PLACEHOLDER}, {PLACEHOLDER})",
                ...
                c.execute(
                    f"INSERT INTO room_members (room_id, username) VALUES ({PLACEHOLDER}, {PLACEHOLDER})",
        ...
        WHERE room_members.username = {PLACEHOLDER}
        GROUP BY rooms.id
...
def group_chat(room_id):
    ...
    try:
        ...
        # Üyelik kontrol et
        c.execute(
            f"SELECT * FROM room_members WHERE room_id={PLACEHOLDER} AND username={PLACEHOLDER}",
            (room_id, session["user"])
        )
        ...
        if request.method == "POST":
            ...
            if new_member:
                try:
                    ...
                    c.execute(
                        f"SELECT username, banned FROM users WHERE username={PLACEHOLDER}",
                        (new_member,)
                    )
                    user = c.fetchone()
        ...
        # Grup bilgisi
        c.execute(f"SELECT room_name, creator FROM rooms WHERE id={PLACEHOLDER}", (room_id,))
        room = c.fetchone()
        ...
        # Üyeler listesi
        c.execute(
            f"SELECT username FROM room_members WHERE room_id={PLACEHOLDER}",
            (room_id,)
        )
...
def leave_group(room_id):
    ...
    try:
        ...
        c.execute(f"SELECT creator FROM rooms WHERE id={PLACEHOLDER}", (room_id,))
        room = c.fetchone()
        ...
        # Üyelikten kaldır
        c.execute(
            f"DELETE FROM room_members WHERE room_id={PLACEHOLDER} AND username={PLACEHOLDER}",
            (room_id, session["user"])
        )
...
def delete_group(room_id):
    ...
    try:
        ...
        c.execute(f"DELETE FROM rooms WHERE id={PLACEHOLDER}", (room_id,))
...
def delete_group_message(msg_id):
    ...
    try:
        ...
        c.execute(
            f"SELECT room_id, sender FROM room_messages WHERE id={PLACEHOLDER}",
            (msg_id,)
        )
        ...
        # Mesajı sil
        c.execute(f"DELETE FROM room_messages WHERE id={PLACEHOLDER}", (msg_id,))
